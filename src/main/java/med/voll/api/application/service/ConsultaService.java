...
public class ConsultaService implements ConsultaUseCase {
    ...
    public DetalhamentoConsultaResponse agendar(DadosAgendamentoConsultaDTO dados) {
        ...
        if(!pacienteRepository.existePacientePorId(dados.idPaciente())){
            throw new ValidacaoException("Id do paciente informado não existe");
        }

        if(dados.idMedico() != null && !medicoRepositoryImpl.existeMedicoPorId(dados.idMedico())){
            throw new ValidacaoException("Id do médico informado não existe");
        }

        validadores.forEach(validador -> validador.validar(dados));

        var paciente = pacienteRepository.obterPacientePorId(dados.idPaciente());
        ...
        var response = consultaRepository.agendarConsulta(consulta);
    ...
    private MedicoEntity escolherMedico(DadosAgendamentoConsultaDTO dados) {
        ...
        if(dados.idMedico() != null){
            return medicoRepositoryImpl.obterMedicoPorId(dados.idMedico());
        }
        ...
        return medicoRepositoryImpl.escolherMedicoAleatorioLivreNaData(dados.especialidade().name(),dados.data());
    }
}
